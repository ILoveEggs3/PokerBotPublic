...
namespace Amigo.Controllers
{
    public class CGame2MaxBotsOnlyController: CGameController
    {
        ...
        public CGame2MaxBotsOnlyController((CPlayer, CBotPoker) _btnPlayer, (CPlayer, CBotPoker) _bbPlayer, double _smallBlind, double _bigBlind, double _antes, bool _resetStackEveryHand): base(new List<CPlayer>(2) { _btnPlayer.Item1, _bbPlayer.Item1 }, _smallBlind, _bigBlind, _antes, _resetStackEveryHand)
        {
            if (_btnPlayer.Item1.PNumberOfChipsLeft != _btnPlayer.Item1.PNumberOfChipsAtBeginningHand)
            ...
            else if (_bbPlayer.Item1.PNumberOfChipsLeft != _bbPlayer.Item1.PNumberOfChipsAtBeginningHand)
                throw new ArgumentException("The number of chips left must be the same number as the number of chips at the beginning of the hand");

            FFLstBots = new List<CBotPoker>(2) { _btnPlayer.Item2, _bbPlayer.Item2 };

            if (_btnPlayer.Item1.PNumberOfChipsAtBeginningHand > _bbPlayer.Item1.PNumberOfChipsAtBeginningHand)
            {
                _btnPlayer.Item1.PNumberOfChipsAtBeginningHand = _bbPlayer.Item1.PNumberOfChipsAtBeginningHand;
                _btnPlayer.Item1.PNumberOfChipsLeft = _bbPlayer.Item1.PNumberOfChipsLeft;
            }
            else if (_bbPlayer.Item1.PNumberOfChipsAtBeginningHand > _btnPlayer.Item1.PNumberOfChipsAtBeginningHand)
            {
                _bbPlayer.Item1.PNumberOfChipsAtBeginningHand = _btnPlayer.Item1.PNumberOfChipsAtBeginningHand;
                _bbPlayer.Item1.PNumberOfChipsLeft = _btnPlayer.Item1.PNumberOfChipsLeft;
            }

            // Création des joueurs
            FFTabJoueurs = new CPlayer[2] { _btnPlayer.Item1, _bbPlayer.Item1 };            
        }
        ...
        protected override void ReceivedAction(CAction _action)
        {
            ...
            switch (_action.PAction)
            {
                case PokerAction.Fold:
                    LFFold();
                    FFCurrentGameState = FFCurrentGameState.Fold();
                    break;
                case PokerAction.Check:
                    LFCheck();
                    FFCurrentGameState = FFCurrentGameState.Check();
                    break;
                case PokerAction.Call:
                    LFCall();
                    FFCurrentGameState = FFCurrentGameState.Call();
                    break;
                case PokerAction.Bet:
                    LFBet();
                    FFCurrentGameState = FFCurrentGameState.Bet(_action.PMise);
                    break;
                case PokerAction.Raise:
                    LFRaise();
                    FFCurrentGameState = FFCurrentGameState.Raise(_action.PMise);
                    break;
            }
            #endregion
            ...
            void LFChangerStadeTour()
            {
                ...
                switch (PCurrentStreet)
                {
                    case Street.Flop:
                        ...
                        int playerOneIndex = FFCurrentGameState.PActionList[0].Item2;
                        int playerTwoIndex = FFCurrentGameState.PActionList[1].Item2;

                        if (playerOneIndex == playerTwoIndex)
                            throw new Exception();

                        string playerOneCards = null;
                        string playerTwoCards = null;

                        if (FFCurrentGameState.PPlayerList[playerOneIndex].PName == FFTabJoueurs[0].PName)
                        {
                            playerOneCards = FFTabJoueursCartes[0].Replace(" ", "");
                            playerTwoCards = FFTabJoueursCartes[1].Replace(" ", "");
                        }
                        else
                        {
                            playerOneCards = FFTabJoueursCartes[1].Replace(" ", "");
                            playerTwoCards = FFTabJoueursCartes[0].Replace(" ", "");
                        }

                        CDBHelperHandInfos.LoadBoardInfosAsync(Hand.ParseHand((string)string.Concat(carte1, carte2, carte3).Clone()), Hand.ParseHand((string)playerOneCards.Clone()), playerOneIndex);
                        CDBHelperHandInfos.LoadBoardInfosAsync(Hand.ParseHand((string)string.Concat(carte1, carte2, carte3).Clone()), Hand.ParseHand((string)playerTwoCards.Clone()), playerTwoIndex);                        
                        break;
                    case Street.Turn:
        ...
        protected override sealed void Play()
        {
            ...
            void MakeDecision()
            {
                ...
                CAction decision = FFLstBots[PIndJoueurActuel].GetDecision(FFCurrentGameState, botCards, botPlayerIndex);
