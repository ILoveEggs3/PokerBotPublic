...
namespace HoldemHand
{
    public partial class Hand : IComparable
    {
        ...
        /*public static List<Tuple<ulong, double>> HandStrengthList(ulong _board, ulong _deadCards = 0)
        {
            //HandStrength optimised for handList queries

            var boardCardCount = BitCount(_board);
            if (boardCardCount < 3 || boardCardCount > 5)
                throw new ArgumentException("board must have 3, 4 or 5 cards for this calculation");

            var capacity = (52 - boardCardCount) * (52 - boardCardCount - 1) / 2;
            var handList = new List<Tuple<ulong, uint>>(capacity);
            var idxRangeLst = new List<Tuple<int, int>>();
            foreach (ulong pocket in Hand.Hands(0, _board | _deadCards, 2))
            {
                handList.Add(new Tuple<ulong, uint>(pocket, Hand.Evaluate(pocket | _board)));
            }
            handList.Sort((x, y) =>
            {
            ...
            {
                ret.Add(new Tuple<ulong, double>(handList[i].Item1, 1.0 - (1.0 * i / capacity)));
            }
       ...
       /// <returns>True means approximate results</returns>
        ...
        /// <param name="_opponentRange">Opponent range to iterate through to get the hand potential. The ulong are his hands masks.</param>
        public static void HandPotential(ulong pocket, ulong board, out double ppot, out double npot, IEnumerable<ulong> _opponentRange)
        {
            const int ahead = 2;
            const int tied = 1;
            const int behind = 0;
            ...
            foreach (ulong oppPocket in _opponentRange)
            {
        ...
        #region Part added by us
        /// <summary>
        /// Returns if there's a flush possible on the board given.
        /// </summary>
        /// <returns></returns>
        public static bool IsFlushPossible(ulong _board)
        {
            // Seperate out by suit
            uint sc = (uint)((_board >> (CLUB_OFFSET)) & 0x1fffUL);
            uint sd = (uint)((_board >> (DIAMOND_OFFSET)) & 0x1fffUL);
            uint sh = (uint)((_board >> (HEART_OFFSET)) & 0x1fffUL);
            uint ss = (uint)((_board >> (SPADE_OFFSET)) & 0x1fffUL);

            // Check if board is 3 suited. 
            bool discountSuitedBoard = ((nBitsTable[sc] > 2) || (nBitsTable[sd] > 2) || (nBitsTable[sh] > 2) || (nBitsTable[ss] > 2));

            return discountSuitedBoard;
        }

        /// <summary>
        /// If the hand has a backdoor straight draw
        /// </summary>
        /// <param name="pocket">Pocket mask of the hand</param>
        /// <param name="board">Board mask of the hand</param>
        /// <returns>Returns if the hand received contains a backdoor straight draw with the board.</returns>
        public static bool IsBackDoorStraightDraw(ulong pocket, ulong board)
        {
            ...
            foreach (var card in Hand.Cards(pocket | board))
            {
            ...
            return bStraight && !bOneCardStraight && !bStraightComplete;
        }

        /// <summary>
        /// If the hand has a backdoor flush draw
        /// </summary>
        /// <param name="pocket">Pocket mask of the hand</param>
        /// <param name="board">Board mask of the hand</param>
        /// <returns>Returns if the hand received contains a backdoor flush draw with the board.</returns>
        public static bool IsBackDoorFlushDraw(ulong pocket, ulong board)
        {
            var cardValues = new List<char>() { 'A', '2', '3', '4', '5', '6', '7', '8', '9', 'T', 'J', 'Q', 'K', 'A' };
            ...
            return cardTypesCount.Max(x => x.Value) == 3;
        }

        /// <summary>
        /// Self explanatory
        /// </summary>
        /// <param name="_pocketMask"></param>
        /// <param name="_boardMask"></param>
        /// <returns></returns>
        public static HandTypes GetHandTypeExcludingBoard(ulong _pocketMask, ulong _boardMask)
        {
            var handWithBoardHandValue = Hand.EvaluateType(_pocketMask | _boardMask);

            switch (handWithBoardHandValue)
            {
                case HandTypes.HighCard:
                    return HandTypes.HighCard;
                case HandTypes.TwoPair:
                    var lstCards = Hand.Cards(_pocketMask | _boardMask).Select(x => Hand.CardRank(Hand.ParseCard(x)));
                    var lstPairs = lstCards.Where(x => lstCards.LongCount(y => y == x) == 2).ToList();
                    lstPairs.Sort();

                    var pocketCards = Hand.Cards(_pocketMask).Select(x => Hand.CardRank(Hand.ParseCard(x))).ToList();
                    var pairCount = 0;
                    if (lstPairs.IndexOf(pocketCards[0]) >= (lstPairs.Count - 4))
                        pairCount++;
                    if (lstPairs.IndexOf(pocketCards[1]) >= (lstPairs.Count - 4))
                        pairCount++;
                ...
                case HandTypes.Trips:                    
                    // If handwithBoard.HandTypeValue == handWithoutPocket.HandTypeValue <-- THIS IS THE EQUIVALENT
                    if (handWithBoardHandValue == Hand.EvaluateType(_boardMask))
                        return HandTypes.HighCard;
                    else
                        return handWithBoardHandValue;
                ...
                case HandTypes.StraightFlush:
                    ...
                    if (handWithBoardHandValue > Hand.EvaluateType(_boardMask))
                        ...
                        return HandTypes.HighCard;
                default:
                    throw new Exception("Handtype is incorrect!");
            }
        }
        #endregion
        #endregion
