...
namespace HandHistories.Objects.GameDescription
{
    ...
    public class Limit
    {
        ...
        private Limit(double smallBlind, double bigBlind, Currency currency, bool isAnteTable, double ante)
        {           
            Currency = currency;
        ...
        public static Limit FromSmallBlindBigBlind(double smallBlind,
                                                   double bigBlind, 
                                                   Currency currency, 
                                           ...
                                           double anteAmount = 0)
        {
            ...
            double smallBlind = double.Parse(smallBlindString, System.Globalization.CultureInfo.InvariantCulture) / 100.0;

            string bigBlindString = limitEnum.ToString().Split('_')[2].Replace("c", "");
            double bigBlind = double.Parse(bigBlindString, System.Globalization.CultureInfo.InvariantCulture) / 100.0;

            return FromSmallBlindBigBlind(Convert.ToDouble(smallBlind), Convert.ToDouble(bigBlind), currency, isAnteTable, anteAmount);
        }
        ...
        public static Limit ParseDbSafeString(string limitString)
        {
            ...
            double smallBlind = Int32.Parse(split[0])/100.0;
            double bigBlind = Int32.Parse(split[1]) / 100.0;

            double ante = (split.Length == 4) ? Int32.Parse(split[2])/100.0 : 0.0;
            ...
            return Limit.FromSmallBlindBigBlind(Convert.ToDouble(smallBlind), Convert.ToDouble(bigBlind), currency, ante != 0, Convert.ToDouble(ante));
        }
