...
namespace HandHistories.Parser.Parsers.FastParser._888
{
    public sealed class Poker888FastParserImpl : HandHistoryParserFastImpl
    {
        ...
        protected override TableType ParseTableType(string[] handLines)
        {
            ...
            var tableStack = 0.0;
            var playersBelow10bb = 0.0;
            var playersAbove20bb = 0.0;
            foreach (Player player in playerList)
            {
                tableStack += player.StartingStack;
                if (player.StartingStack / limit.BigBlind == 5) return TableType.FromTableTypeDescriptions(TableTypeDescription.PushFold);
                if (player.StartingStack / limit.BigBlind <= 10) playersBelow10bb++;
                if (player.StartingStack / limit.BigBlind > 29) playersAbove20bb++;
            ...
            if (tableStack / limit.BigBlind / playerList.Count <= 17.5)
            {
                if (isjackPotTable)
                {
        ...
        protected override Limit ParseLimit(string[] handLines)
        {
            ...
            double lowLimit = ParseAmount(lowLimitString);
            double highLimit = ParseAmount(highLimitString);
        ...
        private static double ParseAmount(string amountString)
        {
            ...
            {
                result += double.Parse(amount, NumberStyles.AllowCurrencySymbol | NumberStyles.Number, NumberFormatInfo);
            }

            return result;
        }

        static double ParseDeadBlindAmount(string amountString)
        {
            ...
            double amount1 = ParseAmount(amountStr1);
            double amount2 = ParseAmount(amountStr2);
        ...
        protected override PlayerList ParsePlayers(string[] handLines)
        {
            ...
            for (int i = 0; i < seatCount; i++)
            {
                ...
                double amount = ParseAmount(handLine.Substring(openParenIndex + 3, handLine.Length - openParenIndex - 3 - 2));
        ...
        private List<HandAction> FixUncalledBets(List<HandAction> handActions, double? totalPot, double? rake)
        {
            ...
            if (handActions.Sum(a => a.Amount) == 0)
                return handActions;
            ...
            foreach (var actions in playerActions)
            {
                ...
                if (lastAction.HandActionType == HandActionType.BET || lastAction.HandActionType == HandActionType.RAISE)
                {
                    ...
                    var totalInvestedAmountOtherPlayer = 0.0;
