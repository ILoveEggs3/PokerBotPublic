...
namespace HandHistories.Parser.Parsers.FastParser.PartyPoker
{
    public sealed class PartyPokerFastParserImpl : HandHistoryParserFastImpl
    {
        ...
        protected override Limit ParseLimit(string[] handLines)
        {
            ...
            {
                double limitBB = double.Parse(limitSubstring, System.Globalization.CultureInfo.InvariantCulture);
                return Limit.FromSmallBlindBigBlind(limitBB / 2.0, limitBB, currency);
            }
        ...
        {
            double bigblind = double.Parse(limitSubstring, NumberStyles.AllowCurrencySymbol | NumberStyles.Number, NumberFormatInfo) *0.4;
        ...
        {
            double buyIn = double.Parse(limitSubstring, NumberStyles.AllowCurrencySymbol | NumberStyles.Number, NumberFormatInfo);
            double bigBlind = buyIn / 100.0;

            if (bigBlind == 0.25)
            {
                return Limit.FromSmallBlindBigBlind(0.10, 0.25, currency);
            }

            return Limit.FromSmallBlindBigBlind(bigBlind / 2.0, bigBlind, currency);
        }
        ...
        static Limit ParseNormalLimit(string limitSubstring, Currency currency)
        {
            ...
            double small = double.Parse(SB, NumberStyles.AllowCurrencySymbol | NumberStyles.Number, NumberFormatInfo);
            double big = double.Parse(BB,NumberStyles.AllowCurrencySymbol | NumberStyles.Number, NumberFormatInfo);
        ...
        static HandAction ParseWinsAction(string line, int potID = 0)
        {
            ...
            double amount = ParseDecimal(line, amountStartIndex);
        ...
        static HandAction ParseActionWithSize(string line, Street currentStreet)
        {
            ...
            double amount = ParseDecimal(line, amountStartIndex + 2);
        ...
        static HandAction ParseDotAction(string line, Street street)
        {
            ...
            else if (line.Contains(" shows"))
            {
                if (isWinType(line, " shows [", ref playerNameIndex))
                {
                    ...
                    return new HandAction(playerName, HandActionType.SHOW, 0, street);
                }
                else if (line.Contains(" for low."))
                {
                    playerNameIndex = line.IndexOf(" shows");
                    playerName = line.Remove(playerNameIndex);
                    return new HandAction(playerName, HandActionType.SHOWS_FOR_LOW, 0, street);
                }
                else
                {
            ...
            else if (line.Contains(" does not "))
            {
                ...
                playerName = line.Remove(playerNameIndex);
                return new HandAction(playerName, HandActionType.MUCKS, 0, street);
            }
        ...
        static double ParseDecimal(string line, int startIndex)
        {
            int endIndex = line.IndexOf(' ', startIndex);
            if (endIndex == -1)
            ...
            return double.Parse(text, NumberStyles.AllowCurrencySymbol | NumberStyles.Number, NumberFormatInfo);
        }
        ...
        static HandAction ParseActionWithoutSize(string line, Street street)
        {
            ...
            {
                case 'd':
                    return new HandAction(ParseActionPlayerName(line, 6), HandActionType.FOLD, 0, street);
                case 'k':
                    return new HandAction(ParseActionPlayerName(line, 7), HandActionType.CHECK, 0, street);
                default:
        ...
        protected override PlayerList ParsePlayers(string[] handLines)
        {
            ...
            for (int lineNumber = playerListStart; lineNumber < handLines.Length; lineNumber++)
            {
                ...
                if (playerNameStartIndex > 1 && lastChar == ')' && line.StartsWith("Seat "))
                {
                    ...
                    double stack = ParseDecimal(line, openParenIndex + 3);
                ...
                else if (lastChar == '.')
                {
                    ...
                    int seatNumber = -1;
                    double stack = 999999.00; // we make the stacksize very high so bettings can never result in a "negative stack"
