    ...
    public sealed class OnGameFastParserImpl : HandHistoryParserFastImpl
    {
        ...
        protected override void ParseExtraHandInformation(string[] handLines, HandHistorySummary handHistory)
        {
            ...
            for (int i = handLines.Length - 1; i >= 0; i--)
            {
                ...
                if (handLine[0] == 'R')
                {
                    handHistory.Rake = double.Parse(handLine.Substring(12), NumberStyles.AllowCurrencySymbol | NumberStyles.Number, _numberFormatInfo);
                }
                ...
                else if (handLine[1] == 'i' || handLine[0] == 'M')
                {
                    ...
                    handHistory.TotalPot += double.Parse(handLine.Substring(colonIndex + 2, wonIndex - colonIndex - 2), NumberStyles.AllowCurrencySymbol | NumberStyles.Number, _numberFormatInfo);
                }
        ...
        protected override Limit ParseLimit(string[] handLines)
        {
            ...
            double smallBlind = double.Parse(sbstring, NumberStyles.AllowCurrencySymbol | NumberStyles.Number, _numberFormatInfo);
            double bigBlind = double.Parse(handLines[2].Substring(slashIndex + 1, commaIndex - slashIndex - 1), NumberStyles.AllowCurrencySymbol | NumberStyles.Number, _numberFormatInfo);
        ...
        protected override List<HandAction> ParseHandActions(string[] handLines, GameType gameType = GameType.Unknown)
        {
            ...
            for (int i = startOfActionsIndex; i < handLines.Length; i++)
            {
                ...
                if (currentStreet == Street.Showdown)
                {
                    ...
                    if (handLine.StartsWith("Main pot:"))
                    {
                        ...
                        foreach (var winner in splitted)
                        {
                            ...
                            double amount = double.Parse(winner.Substring(openParenIndex + 1, winner.Length - openParenIndex - 2), NumberStyles.AllowCurrencySymbol | NumberStyles.Number, _numberFormatInfo);
                ...
                if (handLine[handLine.Length - 1] == ')' && currentStreet != Street.Showdown)
                {
                    ...
                    double amount = double.Parse(handLine.Substring(openParenIndex + 1, handLine.Length - openParenIndex - 2), NumberStyles.AllowCurrencySymbol | NumberStyles.Number, _numberFormatInfo);
                ...
                else
                {
                    ...
                    string playerName;
                    double amount = double.Parse(amountstr, NumberStyles.AllowCurrencySymbol | NumberStyles.Number, _numberFormatInfo);
        ...
        private List<HandAction> FixUncalledBets(List<HandAction> handActions, double? totalPot, double? rake)
        {
            ...
            if (totalPot != null && rake != null)
            {
                ...
                if (Math.Abs(returnAmount) <= Math.Abs(playerInvested) && returnAmount != 0.0)
