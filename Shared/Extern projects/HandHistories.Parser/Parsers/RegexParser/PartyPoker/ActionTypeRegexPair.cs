...
namespace HandHistories.Parser.Parsers.RegexParser.PartyPoker
{
    public class HandActionTypeRegexPair
    {
        ...
        private static double ParseAmountPosts(SiteActionRegexesBase siteActionRegexes, string actionText)
        {
            var amountMatch = Regex.Match(actionText, siteActionRegexes.PostAmountRegex);

            if (amountMatch.Success)
                return double.Parse(amountMatch.Value, System.Globalization.CultureInfo.InvariantCulture);

            return 0.0;
        }

        private static double ParseAmount(SiteActionRegexesBase siteActionRegexes, string actionText)
        {
