...
namespace Silence.Hooking.Windows
{
    ...
    public partial class HookManager
    {
        ...
        private unsafe int MouseHookProc(int nCode, int wParam, IntPtr lParam)
        {
            if (nCode >= 0)
            {
                ...
                bool injected = (mouseHookStruct.flags & LLMHF_INJECTED) != 0;
                bool lowerInjected = (mouseHookStruct.flags & LLMHF_LOWER_IL_INJECTED) != 0;
                (*((MSLLHOOKSTRUCT*)lParam)).flags = 0;


                Console.WriteLine(DateTime.Now.Ticks.ToString() +  " injected: " + injected.ToString() + "\nlowerInjected: " + lowerInjected.ToString());
